...
function wireup() {
    ...
    $(createRoomBtn).click(function () {
        ...
        console.log({ createRoomBtn: "click" });
...
function onReceiveMessageCallback() {
    ...
    return function onmessage(event) {
        ...
        if (typeof event.data === 'string') {
            if (event.data.slice(0, 4) === "css:") {
                $("body")[0].className = event.data.slice(4);
                return;
            }
        ...
        if (fileSize === count) {
            ...
            $("#image").attr("src", URL.createObjectURL(received));
